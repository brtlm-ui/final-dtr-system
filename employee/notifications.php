...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'mark_read') {
    ...
    try {
        ...
        foreach ($ids as $id) {
            ...
            if (!$row) continue;
            if ($row['user_id'] === null) {
                // broadcast => track per-employee in notification_reads
                $insertNr->execute([$id, $empId]);
            } else {
                if ((int)$row['user_id'] === (int)$empId) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'mark_all_read') {
    try {
        ...
        foreach ($rowsAll as $row) {
            if ($row['user_id'] === null) {
                $insertNr->execute([$row['notification_id'], $empId]);
            } else if ((int)$row['user_id'] === (int)$empId) {
...
// Fetch notifications relevant for employee (direct + broadcast)
$empId = $_SESSION['employee_id'];
// Fetch notifications relevant for employee (direct + broadcast), show per-user read state for broadcasts
...
<div class="container-fluid mt-4">
    ...
    <?php
    function formatNotificationEmployee($row) {
        $type = $row['type'];
        $raw = $row['payload'];
        $payload = $raw;
        if (is_string($raw)) {
        ...
        if (!is_array($payload)) return [$type, $raw];
        $friendly = '';
        switch ($type) {
            case 'reason_submitted':
                ...
                $friendly = "Your edit request for record #$rec ($entryLabel) was submitted";
                break;
            case 'reason_approved':
                $rec = $payload['record_id'] ?? '';
                $entry = $payload['entry_type'] ?? '';
                $entryLabel = strtoupper(str_replace('_',' ',$entry));
                $friendly = "Approved: record #$rec ($entryLabel)";
                break;
            case 'reason_rejected':
                $rec = $payload['record_id'] ?? '';
                $entry = $payload['entry_type'] ?? '';
                $entryLabel = strtoupper(str_replace('_',' ',$entry));
                $friendly = "Rejected: record #$rec ($entryLabel)";
                break;
            case 'day_complete':
                $date = $payload['date'] ?? '';
                $friendly = "All clock events completed for $date";
                break;
            default:
    ...
    <form method="POST" id="notifForm">
        ...
        <div class="mb-3">
            ...
            <?php else: ?>
            <?php foreach ($rows as $r): list($friendly,$details)=formatNotificationEmployee($r); ?>
